# AGENT.md - Master Document for AI Agents

This file serves as the master document for all AI agents working with the Voice Studio repository. All other guidance documents must be based on this master document.
...
**ML Note**: Neural denoising (DTLN) is now a core part of the plugin and is always compiled in. It uses optimized CPU inference for cross-platform stability. GPU acceleration has been removed to ensure a single stable binary.
...
**Pre-Processing**:
- `src/dsp/speech_hpf.rs`: 90Hz hygiene filter.
- `src/dsp/speech_confidence.rs`: Analysis sidechain.

**Restoration stage**:
- `src/dsp/denoiser.rs`: Hybrid Spectral (DSP) + Neural (DTLN) denoiser.
- `src/dsp/plosive_softener.rs`: Automatic thump protection.
- `src/dsp/breath_reducer.rs`: Confidence-weighted breath softening.
- `src/dsp/deverber.rs`: Envelope-based late reverb reduction.
...
- Leveler: linked stereo leveling compression.
- Limiter: output safety limiting.

### Key Modules

- `src/lib.rs` - Plugin entry point, `process()` loop, parameter definitions.
- `src/dsp/mod.rs` - DSP chain orchestration.
- `src/macro_controller.rs` - Simple mode macro-to-parameter mapping.
- `src/ui.rs` - Vizia-based GUI with Simple/Advanced toggles and Reset button.
- `src/debug.rs` - Centralized `vs_log!` infrastructure (feature-gated).
- `src/meters.rs` - Thread-safe atomic metering.
- `src/presets.rs` - Final Output Presets functionality.

### Macro Controller

Intent-based "Simple mode" that maps user-facing macros to underlying parameters:
- **Distance** → reverb_reduction, proximity.
- **Clarity** → noise_reduction, de_esser, noise_tone, breath_control scaling.
- **Consistency** → leveler.

### Global Reset

The plugin includes a global reset mechanism triggered from the UI header. It resets all DSP state (filters, buffers, neural states) and returns all parameters to their default values.
...
- All divisions must have epsilon guards (`1e-12`).
- All time-constant calculations must guard against zero sample rates.
- `catch_unwind` wraps all FFI entry points to prevent host crashes.

### No Delta Preview

The "Preview Cuts" / Delta listening system has been completely removed. Restoration modules no longer compute or expose energy deltas to the UI.
...
## Master Document Instructions

This file (AGENT.md) is the master document. All other guidance files in the repository must be based on this document:
- `CLAUDE.md`
- `GEMINI.md`
- `QWEN.md`

AI agents should refer to this document as the authoritative source for project information.
